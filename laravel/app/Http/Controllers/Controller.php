...
class Controller extends BaseController
{
    ...
    public function create(Request $request)
    {
        ...
        $is_direct_post = null;
        if (array_key_exists('is_direct_post', $inputs)) {
            $is_direct_post = $inputs['is_direct_post'];
        }

        $price = (float) $inputs['product_price'] * 100;
        $price_in_cent = (int) $price;

        // create order
        $order = new Order();
        $order->product_name = $inputs['product_name'];
        $order->product_price = $price_in_cent; // to get value in cents
        $order->customer_email = $inputs['client_email'];
        $order->save();
        ...
        $purchase->success_redirect = 'http://localhost?status=success';
        $purchase->failure_redirect = 'http://localhost?status=fail';
